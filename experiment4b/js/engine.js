...
function draw() {
  //player to camera offset written with help of ChatGPT
  //calc player worldPos
  let playerWorldPos = window.getPlayerPos();
  let playerWorldX = playerWorldPos[0];
  let playerWorldY = playerWorldPos[1];
  /*
  //convert pos to cam coords
  let playerCameraPos = worldOffsetToCamera([playerWorldX, playerWorldY]);
  
  //update camera offset
  camera_offset.x = -playerCameraPos.x + width / 2;
  camera_offset.y = playerCameraPos.y - height / 2;*/
  console.log(camera_offset.x + ", " + camera_offset.y);
  console.log(playerWorldX + ", " + playerWorldY);
  ...
  if (currentTime - lastUpdateTime >= 500) { // 1000 ms = 1 second
    camera_velocity.y += 0.5;
    camera_velocity.x -= 1;
  ...
  if (!isVisible([playerWorldX, playerWorldY], [camera_offset.x, camera_offset.y])) {
    noLoop();
...
function isVisible(worldPos, cameraPos) {
  let [screenX, screenY] = worldToScreen(worldPos, cameraPos);
  
  screenX = 0 - screenX; // Apply the same flip as your drawTile code
